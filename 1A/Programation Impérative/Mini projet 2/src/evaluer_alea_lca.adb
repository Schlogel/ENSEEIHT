...
procedure Evaluer_Alea_LCA is
	...
	is
		...
		package LCA_Objets is new LCA (T_Cle => Integer, T_Donnee => Integer);
		
		Sda : LCA_Objets.T_LCA;
	...
	begin
		Max := 1;
		Min := Taille;
		LCA_Objets.Initialiser(Sda);
		for index in 1..Taille loop
			Donnee := 0;
			Get_Random_Number (Nombre_Aleatoire);
			LCA_Objets.Enregistrer(Sda, Nombre_Aleatoire, LCA_Objets.La_Donnee_Better(Sda,Nombre_Aleatoire,Donnee) + 1);
		end loop;
		
		for Numero in 1..Borne loop
			Donnee := LCA_Objets.La_Donnee_Better(Sda,Numero,Donnee);
			if Min > Donnee then
				Min := Donnee;
			else
				Null;
